...
import { addRecipeRequest, deleteRecipeRequest, updateRecipeRequest } from '../../store/modules/recipe/actions';
...
export default function Form() {
	...
	const [category, setCategory] = useState(selectedRecipe.category !== undefined ? selectedRecipe.category.id : '');
	...
	return (
		<StyleForm initialData={selectedRecipe} schema={schema} onSubmit={handleSubmit}>
			{selectedRecipe.user !== undefined && <GoTrashcan className="btn-delete" onClick={handleDelete} />}
			<div className="row">
				...
				<div className="col-12">
					<Input name="description" multiline type="text" label={'Descrição'} placeholder={``} />
				</div>
				<button type="submit">
					{selectedRecipe.user !== undefined ? 'Atualizar Receita' : 'Criar Receita'}
				</button>
			</div>
		</StyleForm>
	);
